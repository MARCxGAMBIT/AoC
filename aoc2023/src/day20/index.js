...
class Conjunction {
    ...
    send() {
        const pulse = Object.entries(this.memory).every(([_, pulse]) => pulse === "H") ? "L" : "H";
        for (let destination of this.destinations) {
            this.queue.push([this.name, destination, pulse]);
        }
    }
...
const part1 = (rawInput) => {
    ...
    const config = input.reduce((acc, line) => {
        ...
        if (module[0] === "%") {
            acc[module.substring(1)] = new FlipFlop(module.substring(1), destinations.split(", "), acc, queue);
        } else if (module[0] === "&") {
            acc[module.substring(1)] = new Conjunction(module.substring(1), destinations.split(", "), acc, queue);
        } else {
            acc[module] = new BroadCaster(module, destinations.split(", "), acc, queue);
        }
        return acc;
    }, {});
...
const part2 = (rawInput) => {
    ...
    while (true) {
        ...
        while (queue.length > 0) {
            const [sender, receiver, pulse] = queue.shift();
            if (magicSenders.includes(sender) && pulse === "H" && minCycles[sender] === -1) {
