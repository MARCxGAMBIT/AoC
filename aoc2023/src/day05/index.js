...
const generateMappingFunctions = (categoryRanges) => (input) => {
  const isInputInRange = ([_, source, len]) =>
    source <= input && input <= source + len - 1;
  const range = categoryRanges.find(isInputInRange);
  if (!range) {
    return input;
  }
...
const findNumbersInString = (str) =>
  [...str.matchAll(/\d+/g)].map(convertMatchToNumber);

const findLocation = (mapping) => (seed) =>
  mapping.reduce((acc, func) => func(acc), seed);
...
const part1 = (rawInput) => {
  ...
  const locations = findNumbersInString(seedsInfo).map(convertSeedToLocation);
