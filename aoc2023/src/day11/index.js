...
const transpose = (universe) =>
  universe[0].map((_, colIndex) => universe.map((row) => row[colIndex]));
...
const diffBetweenCoords = (a, b, emptyRows, emptyCols, expansion) => {
  ...
  const expandedRows = emptyRows.filter(
    (row) => row > minRow && row < maxRow,
  ).length;
  const expandedCols = emptyCols.filter(
    (col) => col > minCol && col < maxCol,
  ).length;
  return (
    Math.abs(a.row - b.row) +
    Math.abs(a.col - b.col) +
    (expandedRows + expandedCols) * (expansion - 1)
  );
};
...
const shortestDistances = (universe, expansion) => {
  const emptyRows = universe.map(indexIfEmpty).filter(notEmpty);

  const emptyCols = transpose(universe).map(indexIfEmpty).filter(notEmpty);
    ...
    coords
      .map((coord, i, array) =>
        array
          .slice(i + 1)
          .map((other) =>
            diffBetweenCoords(coord, other, emptyRows, emptyCols, expansion),
          )
          .reduce(sum, 0),
      )
      .reduce(sum, 0)
