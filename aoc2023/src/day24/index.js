...
const part1 = (rawInput) => {
    ...
    for (let outer = 0; outer < hailstones.length - 1; outer++) {
        ...
        for (let inner = outer + 1; inner < hailstones.length; inner++) {
            ...
            const intersection = getIntersection(x1, y1, x1 + vx1, y1 + vy1, x2, y2, x2 + vx2, y2 + vy2);
            if (intersection.x >= low && intersection.y >= low && intersection.x <= high && intersection.y <= high) {
                if ((vx1 < 0 && intersection.x < x1 || vx1 > 0 && intersection.x > x1) && (vx2 < 0 && intersection.x < x2 || vx2 > 0 && intersection.x > x2)) {
                    intersectionCounter++;
                }
            }
        }
...
const part2 = async (rawInput) => {
    const input = parseInput(rawInput);
    const hailstones = input.map(line => line.match(/-?\d+/g).map(Number)).slice(0,3);
