...
const getStartConfig = (input) => {
  ...
  const isTopConnected = ["F", "7", "|"].includes(
    input[startRow - 1]?.[startCol],
  );
  const isLeftConnected = ["L", "F", "-"].includes(
    input[startRow]?.[startCol - 1],
  );
  const isBottomConnected = ["J", "L", "|"].includes(
    input[startRow + 1]?.[startCol],
  );
  const isRightConnected = ["J", "7", "-"].includes(
    input[startRow]?.[startCol + 1],
  );
...
const part1 = (rawInput) => {
  ...
  let neighbor = getUnvisitedNeighbor(
    input,
    startRow,
    startCol,
    visited,
...
const part2 = (rawInput) => {
  ...
  return (
    visited
      .map((row) =>
        row.filter((cell) => cell.toString() === "0" || cell === "x").join(""),
      )
